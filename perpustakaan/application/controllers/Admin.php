...
class Admin extends CI_Controller{
	...
	function laporan_pdf_buku(){
		...
		$paper_size = 'A4';                                                      // ukuran kertas  //tipe format kertas potrait atau landscape
	...
	function laporan_print_transaksi(){
		...
		if($dari != "" && $sampai != ""){
			$data['laporan'] = $this->db->query("select * from transaksi t,buku b,anggota a 
			where t.id_buku=b.id_buku and t.id_anggota=a.id_anggota and 
			date(tgl_pencatatan) >= '$dari'")->result();
	...
	function laporan_pdf_transaksi(){
		...
		$data['laporan'] = $this->db->query("select * from transaksi t,buku b,anggota a 
		where t.id_buku=b.id_buku and t.id_anggota=a.id_anggota and date(tgl_pencatatan)
		>= '$dari'")->result();
